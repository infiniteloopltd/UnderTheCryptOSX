...
function UI(cb) {
    ...
    this.tiles.src = 'Icons.png'; // Fiach
...
UI.prototype = {
     ...
     */
    getScale: function () {
		// Added 200 px increase in height
        var scale,
    ...
    drawTitle: function(ctx, scale) {
        ...
        // ctx.drawImage(this.tiles, 0, 32, 16, 16, 65, 184, 16, 16);
		 this.drawSprite(ctx, 2, 2, 16 + 3 * 16, 112 + 3 * 16); // FIACH
    },
    ...
    drawMenu: function (ctx) {
        ...
        if (g.selTower) {
            ...
            for (i = 0; i < 3; i += 1) {
                ...
                if (disabled) {
                    //ctx.globalAlpha = 0.5;
					// BUG: When not enough money to buy a defence tower, then it should go disabled.
					// use DrawSprite instead
                    //ctx.drawImage(this.tiles, 48, 0, 16, 15, i * 32, height + 1, 16, 15); // BUG? 
                    //ctx.drawImage(this.tiles, 48, 0, 16, 15, i * 32, height + 16, 16, 15); // BUG?
                    //ctx.globalAlpha = 1;
					this.drawSpriteGhosted(ctx, i + 1, 1, i * 32, height);
                }
            }
        }
        // tools
    ...
    update: function () {
        ...
        if (redraw) {
            ...
            full = g.save.full ? 'Fullscreen' : 'Compact';
            for (i = 0; i < 5; i += 1) {
    ...
    drawSprite: function(ctx, sx, sy, x, y) {
        ctx.drawImage(this.tiles, sx * 160, sy * 160, 160, 160, x, y, 16, 16);// FIACH
     ...
     * @param {number} y y coordinate in pixels.
     */
    drawSpriteGhosted: function(ctx, sx, sy, x, y) {
		ctx.globalAlpha = 0.5;
        ctx.drawImage(this.tiles, sx * 160, sy * 160, 160, 160, x, y, 16, 16);// FIACH
        ctx.globalAlpha = 1;
		// ctx.drawImage(this.tiles, sx * 16, sy * 16, 16, 16, x, y, 16, 16);
